...
const samples: { taskMs: number; layoutReads: number; elapsedMs: number }[] = [];
try {
  for (let round = 0; round < 8; round++) {
    ...
    await page.evaluate(async () => {
      window.mountNumber({ value: 11111, locales: "en-US", duration: 500, format: { minimumIntegerDigits: 5, maximumFractionDigits: 0, useGrouping: false } });
      document.getElementById("number")!.style.font = "144px/1.2 monospace";
    ...
    const observed = await page.evaluate(async () => {
      ...
      try {
        ...
        return { layoutReads, elapsedMs: performance.now() - start };
    ...
    const result = { taskMs: (task(after) - task(before)) * 1000, ...observed };
    if (round) samples.push(result);
    console.log(`${round ? `round ${round}` : "warmup"}: ${result.taskMs.toFixed(2)} ms task, ${result.layoutReads} layout reads`);
    await page.close();
  }
  ...
  const result = {
    ...
    workload: { counters: 1, updates: 90, intervalMs: 33, settleMs: 650, font: "144px/1.2 monospace", motionBlur: false, mode: "Vite development server", warmups: 1, rounds: 7 },
    environment: { browser: await browser.version(), bun: Bun.version, platform: process.platform, arch: process.arch, cpu: cpus()[0]?.model },
    medianTaskMs: median(samples.map((sample) => sample.taskMs)),
    medianLayoutReads: median(samples.map((sample) => sample.layoutReads)), samples,
  };
  await Bun.write(process.env.BENCH_OUTPUT ?? "perf/ticker.local.json", `${JSON.stringify(result, null, 2)}\n`);
  console.log(`METRIC ticker_task_ms=${result.medianTaskMs.toFixed(2)} layout_reads=${result.medianLayoutReads}`);
